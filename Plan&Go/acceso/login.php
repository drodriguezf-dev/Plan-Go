...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($email) || empty($password2)) {
        ...
        try {
            ...
            $select = "SELECT id, nombre, activo, password, perfil_id FROM usuarios WHERE email = :email;";
            ...
            if (!$resultado) {
                ...
                if ((int) $resultado["activo"] !== 1) {
                    $errores[] = "Tu cuenta aún no está activa. Revisa tu bandeja de correo.";
                } else {
                    //-----------------------------------------------------
...
<body class="flex items-center justify-center h-screen bg-gray-100">
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        <h1 class="text-2xl font-bold text-gray-700 text-center mb-4">Iniciar sesión</h1>
        ...
        <?php if (isset($_REQUEST["registrado"])): ?>
            <p class="bg-green-100 text-green-700 p-3 rounded mb-4">
                ¡Gracias por registrarte! Revisa tu bandeja de correo para activar la cuenta.
            </p>
        <?php endif; ?>
        <?php if (isset($_REQUEST["activada"])): ?>
            <p class="bg-blue-100 text-blue-700 p-3 rounded mb-4">
                ¡Cuenta activada! Ya puedes iniciar sesión.
            </p>
        <?php endif; ?>
        <?php if (isset($_REQUEST["restablecido"])): ?>
            <p class="bg-blue-100 text-blue-700 p-3 rounded mb-4">
                ¡Tu contraseña ha sido restablecida! Ya puedes iniciar sesión.
            </p>
        <?php endif; ?>
        <?php if (isset($_REQUEST["sesion_expirada"])): ?>
            <p class="bg-yellow-100 text-yellow-700 p-3 rounded mb-4">
                Tu sesión ha expirado. El usuario ya no existe.
            </p>
        <?php endif; ?>
        ...
        <div class="mt-4 text-center">
            <a href="../registro/registro.php" class="text-blue-600 hover:underline">Crear cuenta</a> |
            <a href="../registro/recordar.php" class="text-blue-600 hover:underline">¿Olvidó su contraseña?</a>
            <br>
            <a href="../index.php" class="text-blue-600 hover:underline">Continuar como invitado</a> |
            <a href="./login_gestor.php" class="text-blue-600 hover:underline">Iniciar como trabajador</a>
        </div>
    </div>
</body>
