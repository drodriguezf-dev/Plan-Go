...
try {
    ...
    // Borrar actividad
    $consulta = "DELETE FROM ofertas WHERE id = :oferta_id";
    $stmt = $conexion->prepare($consulta);
    $stmt->execute([":oferta_id" => $oferta_id]);

    // Redirigir de nuevo al listado
    if ($_SESSION['perfil_id'] == 3) {
        header("Location: ../usuarios/actividades_creadas.php");
    ...
    $mensaje_error = "No ha sido posible borrar la oferta. Intenta nuevamente.";
